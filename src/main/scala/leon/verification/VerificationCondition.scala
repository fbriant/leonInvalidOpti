...
case class VCResult(status: VCStatus, solvedWith: Option[Solver], timeMs: Option[Long]) {
  ...
  def report(vctx: VerificationContext) {
    ...
    status match {
      ...
      case VCStatus.Invalid(cex,alt,post) =>
        def printCounterExample(model : Map[Identifier, Expr]) : Unit = {
        ...
        alt match {
          case Some(m) => {
            reporter.error("Following counter-example is also applicable :")
            printCounterExample(m)
          }
          case _ =>
        }
        ...
        post match {
          case Some(p) => {
            reporter.error("Following post conditions are violated :")
            for (expr <- p) {
              reporter.error("  " + expr)
            }
          }
...
object VCStatus {
  case class  Invalid(cex: Map[Identifier, Expr], alt: Option[Map[Identifier, Expr]], post: Option[List[Expr]]) extends VCStatus("invalid")
